...
/*   Updated: 2021/02/06 17:00:02 by rcarmen          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void			ot_fl_influ(t_specs *stuff, unsigned int n, int nl, int d)
{
	int				i;
	char			z_n;
	...
	if (stuff->precision < 0 && n == 0)
		...
		ft_u_ptf_putnbr_fd(n, 1);
}

static void			mi_fl_influ(t_specs *stuff, unsigned int nbr, int nl, int d)
{
	int				i;
